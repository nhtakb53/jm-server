...
D2R의 `Mods\JMServer\Settings.json`은 인증된 계정·장치 조합별로 별도 저장합니다. 플레이 시작 전에 서버 설정을 내려받고 정상 종료 또는 복구 체크인 때 다시 올립니다. 캐릭터별 키·조작 설정 파일인 `.key/.keyo/.ctl/.ctlo`는 캐릭터 프로필과 함께 체크인합니다. 서버에 최초 설정이 없으면 현재 PC 값을 유지한 뒤 첫 플레이 종료 때 백업합니다. 장치 토큰, 서버 주소, 게임 설치 경로 같은 런처 연결 정보는 동기화하지 않습니다.
