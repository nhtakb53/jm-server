...
D2R의 `savepath: JMServer/` 기능이 게임플레이·그래픽 설정을 `Mods\JMServer\Settings.json`에 직접 분리 저장합니다. 런처는 기본 배틀넷용 `Settings.json`을 복사하거나 교체하지 않으며, 정만서버 설정만 계정·장치 조합별로 서버에 백업합니다. 플레이 시작 시 로컬 설정을 우선하고 정상 종료 또는 복구 체크인 때 서버 백업을 갱신합니다. 캐릭터별 키·조작 설정 파일인 `.key/.keyo/.ctl/.ctlo`는 캐릭터 프로필과 함께 체크인합니다. 서버에 최초 설정이 없으면 현재 PC 값을 유지한 뒤 첫 플레이 종료 때 백업합니다. 장치 토큰, 서버 주소, 게임 설치 경로 같은 런처 연결 정보는 동기화하지 않습니다.
