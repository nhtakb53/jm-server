...
- 정만서버 서버: 계정, 장치 인증, 세이브 프로필 임대, 장치별 개인 설정 동기화, 버전 이력, 감사 기록
...
D2R의 `Mods\JMServer\Settings.json`은 인증된 계정·장치 조합별로 별도 저장합니다. D2R이 게임플레이·그래픽 설정을 기본 저장 폴더에 기록하는 동작에 맞춰, 플레이 중에만 정만서버 설정을 기본 `Settings.json` 위치에 임시 적용합니다. 정상 종료 또는 복구 체크인 때 변경값을 정만서버 전용 파일로 회수하고 원래 본게임 설정을 복원한 뒤 서버에 백업합니다. 캐릭터별 키·조작 설정 파일인 `.key/.keyo/.ctl/.ctlo`는 캐릭터 프로필과 함께 체크인합니다. 서버에 최초 설정이 없으면 현재 PC 값을 유지한 뒤 첫 플레이 종료 때 백업합니다. 장치 토큰, 서버 주소, 게임 설치 경로 같은 런처 연결 정보는 동기화하지 않습니다.
