...
정만서버 프로토콜 v7의 PK 로비는 2인 방 예약, 호스트 IPv4 공유, 만료·갱신을 담당합니다. 공개된 D2RLoader 1.0.1-beta 설정에는 게임 포트 변경이나 자동 참가 인자가 없으므로, 런처가 공개 TCP 15571과 D2R의 로컬 TCP 4000을 양쪽 PC에서 중계합니다. 방 생성·참가 선택은 게임 안에서 직접 수행합니다.
